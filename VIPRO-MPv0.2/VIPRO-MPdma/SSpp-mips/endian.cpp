...
enum endian_t
endian::endian_host_byte_order(void)
{
  int i = 1, *p;
...
enum endian_t
endian::endian_host_word_order(void)
{
  int *p;
  double x = 1.0;
...
enum endian_t
endian::endian_target_byte_order(void)
{
  return Sim->LOADER->ld_target_big_endian ? endian_big : endian_little;
}

/* probe target (simulated program) double word endian format,
   only valid after program has been loaded */
enum endian_t
endian::endian_target_word_order(void)
{
