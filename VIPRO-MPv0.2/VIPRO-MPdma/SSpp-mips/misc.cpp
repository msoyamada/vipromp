...
namespace std { extern "C"{ int errno;  extern long random(void);} }
...
//#ifdef __GNUC__
void
misc::_fatal(const char *file, const char *func, int line, char *fmt, ...)
//#else /* !__GNUC__ */
//void
//fatal(char *fmt, ...)
//#endif /* __GNUC__ */
{
  ...
  fprintf(stderr, "\n");
  //if (hook_fn)
    /*(*hook_fn)*/Sim->Simplescalar->sim_print_stats(stderr);
  exit(1);
}
...
//#ifdef __GNUC__
void
misc::_panic(const char *file, const char *func, int line, char *fmt, ...)
//#else /* !__GNUC__ */
//void
//panic(char *fmt, ...)
//#endif /* __GNUC__ */
...
//#ifdef __GNUC__
void
misc::_warn(const char *file, const char *func, int line, char *fmt, ...)
//#else /* !__GNUC__ */
//void
//warn(char *fmt, ...)
//#endif /* __GNUC__ */
...
//#ifdef __GNUC__
void
misc::_info(char *file, char *func, int line, char *fmt, ...)
//#else /* !__GNUC__ */
//void
//info(char *fmt, ...)
//#endif /* __GNUC__ */
...
void
misc::mysrand(unsigned int seed)	/* random number generator seed */
{
...
/* get a random number */
int
misc::myrand(void)			/* returns random number */
{
#ifndef __alpha
...
misc::mystrdup(char *s)		/* string to duplicate to heap storage */
{
  char *buf;
...
misc::mystricmp(char *s1, char *s2)	/* strings to compare, case insensitive */
{
  unsigned char u1, u2;
...
int
misc::log_base2(int n)
{
  int power = 0;

  if (n <= 0 || (n & (n-1)) != 0)
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "log2() only works for positive power of two values");
...
/* assume bit positions numbered 31 to 0 (31 high order bit), extract num bits
   ...
   00000110 */
unsigned int
misc::extractl(int word,		/* the word from which to extract */
...
char *
misc::myvsprintf(char *obuf, char *format, va_list v)
{
  static char _blanks[] = "                    ";
  ...
  for (;;)
	...
	fixed:
	  ...
	  /* set translate table for digits */
	   (fcode == 'X') ? tab = "0123456789ABCDEF" : tab = "0123456789abcdef";
...
char *
misc::mysprintf(char *obuf, char *format, ...)
{
  /* vararg parameters */
  va_list v;
  va_start(v, format);
...
void
misc::myvfprintf(FILE *stream, char *format, va_list v)
{
  /* temp buffer */
  char buf[512];
...
void
misc::myfprintf(FILE *stream, char *format, ...)
{
  /* temp buffer */
  char buf[512];
...
squad_t
misc::myatosq(char *nptr, char **endp, int base)
{
  char *s, *save;
...
//  extern int errno;
  ...
  if (!nptr || !*nptr)
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "strtoll() passed a NULL string");
  ...
  if (base <= 1 || base > 36)
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "bogus base: %d", base);
...
quad_t
misc::myatoq(char *nptr, char **endp, int base)
{
  char *s, *save;
  int overflow;
...
FILE *
misc::gzopen(char *fname, char *type)
{
  int i;
  char *cmd = NULL, *ext;
  FILE *fd;
