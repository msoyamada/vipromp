...
/*static*/ void
cache::unlink_htab_ent(struct cache_t *cp,		/* cache to update */
...
/*static*/ void
cache::link_htab_ent(struct cache_t *cp,		/* cache to update */
	      struct cache_set_t *set,		/* set containing bkt chain */
...
/*static*/ void
cache::update_way_list(struct cache_set_t *set,	/* set contained way chain */
		struct cache_blk_t *blk,	/* block to insert */
		enum list_loc_t where)		/* insert location */
{
  ...
  if (where == Head)
    ...
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__,"bogus WHERE designator");
}
...
cache::cache_create(char *name,		/* name of the cache */
	...
	//				   tick_t now),
	     unsigned int hit_latency, enum pointer_fn_sim pfs)
{
  struct cache_t *cp;
  struct cache_blk_t *blk;
  int i, j, bindex;
  ...
  if (nsets <= 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"cache size (in sets) `%d' must be non-zero", nsets);
  if ((nsets & (nsets-1)) != 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"cache size (in sets) `%d' is not a power of two", nsets);
  /* blocks must be at least one datum large, i.e., 8 bytes for SS */
  if (bsize < 8)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"cache block size (in bytes) `%d' must be 8 or greater", bsize);
  if ((bsize & (bsize-1)) != 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"cache block size (in bytes) `%d' must be a power of two", bsize);
  if (usize < 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"user data size (in bytes) `%d' must be a positive value", usize);
  if (assoc <= 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"cache associativity `%d' must be non-zero and positive", assoc);
  if ((assoc & (assoc-1)) != 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"cache associativity `%d' must be a power of two", assoc);
//  if (!blk_access_fn)
//    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"must specify miss/replacement functions");
  ...
  cp->name = Sim->MISC->mystrdup(name);
  cp->nsets = nsets;
  cp->bsize = bsize;
  cp->balloc = balloc;
  cp->usize = usize;
...
//  cp->blk_access_fn = blk_access_fn;
  ...
  cp->blk_mask = bsize-1;
  cp->set_shift = Sim->MISC->log_base2(bsize);
  cp->set_mask = nsets-1;
  cp->tag_shift = cp->set_shift + Sim->MISC->log_base2(nsets);
...
cache::cache_char2policy(char c)		/* replacement policy as a char */
{
  switch (c) {
  case 'l': return LRU;
  case 'r': return Random;
  case 'f': return FIFO;
  default: Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"bogus replacement policy, `%c'", c);
  }
}
...
void
cache::cache_config(struct cache_t *cp,	/* cache instance */
...
void
cache::cache_reg_stats(struct cache_t *cp,	/* cache instance */
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  Sim->STATS->stat_reg_formula(sdb, buf, "total number of accesses", buf1, "%12.0f");
  sprintf(buf, "%s.hits", name);
  Sim->stat_reg_counter(sdb, buf, "total number of hits", &cp->hits, 0, NULL);
  sprintf(buf, "%s.misses", name);
  Sim->stat_reg_counter(sdb, buf, "total number of misses", &cp->misses, 0, NULL);
  sprintf(buf, "%s.replacements", name);
  Sim->stat_reg_counter(sdb, buf, "total number of replacements",
		 &cp->replacements, 0, NULL);
  sprintf(buf, "%s.writebacks", name);
  Sim->stat_reg_counter(sdb, buf, "total number of writebacks",
		 &cp->writebacks, 0, NULL);
  sprintf(buf, "%s.invalidations", name);
  Sim->stat_reg_counter(sdb, buf, "total number of invalidations",
		 &cp->invalidations, 0, NULL);
  sprintf(buf, "%s.miss_rate", name);
  sprintf(buf1, "%s.misses / %s.accesses", name, name);
  Sim->STATS->stat_reg_formula(sdb, buf, "miss rate (i.e., misses/ref)", buf1, NULL);
  sprintf(buf, "%s.repl_rate", name);
  sprintf(buf1, "%s.replacements / %s.accesses", name, name);
  Sim->STATS->stat_reg_formula(sdb, buf, "replacement rate (i.e., repls/ref)", buf1, NULL);
  sprintf(buf, "%s.wb_rate", name);
  sprintf(buf1, "%s.writebacks / %s.accesses", name, name);
  Sim->STATS->stat_reg_formula(sdb, buf, "writeback rate (i.e., wrbks/ref)", buf1, NULL);
  sprintf(buf, "%s.inv_rate", name);
  sprintf(buf1, "%s.invalidations / %s.accesses", name, name);
  Sim->STATS->stat_reg_formula(sdb, buf, "invalidation rate (i.e., invs/ref)", buf1, NULL);
}

/* print cache stats */
void
cache::cache_stats(struct cache_t *cp,		/* cache instance */
	    FILE *stream)		/* output stream */
{
...
cache::cache_access(struct cache_t *cp,	/* cache to access */
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  if ((nbytes & (nbytes-1)) != 0 || (addr & (nbytes-1)) != 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"cache: access error: bad size or alignment, addr 0x%08x", addr);

  /* access must fit in cache block */
  if ((addr + nbytes) > ((addr & ~cp->blk_mask) + cp->bsize))
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__,"cache: access error: access spans block, addr 0x%08x", addr);
  ...
  case Random:
    {
      int bindex = Sim->MISC->myrand() & (cp->assoc - 1);
      repl = CACHE_BINDEX(cp, cp->sets[set].blks, bindex);
    }
    break;
  default:
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__,"bogus replacement policy");
  }
  ...
  if (repl->status & CACHE_BLK_VALID)
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
      ...
      if (repl->status & CACHE_BLK_DIRTY)
	{
	  ...
	  cp->writebacks++;
	  switch (cp->pfs){
		  case dl1_access_fn_i:
		  lat += Sim->dl1_access_fn(Write,
				CACHE_MK_BADDR(cp, repl->tag, set),
				cp->bsize, repl, now+lat);
		  break;
		  case dl2_access_fn_i:
		  lat += Sim->dl2_access_fn(Write,
				CACHE_MK_BADDR(cp, repl->tag, set),
				cp->bsize, repl, now+lat);
		  break;
		  case il1_access_fn_i:
		  lat += Sim->il1_access_fn(Write,
				CACHE_MK_BADDR(cp, repl->tag, set),
				cp->bsize, repl, now+lat);
		  break;
		  case il2_access_fn_i:
		  lat += Sim->il2_access_fn(Write,
				CACHE_MK_BADDR(cp, repl->tag, set),
				cp->bsize, repl, now+lat);
		  break;
		  case itlb_access_fn_i:
		  lat += Sim->itlb_access_fn(Write,
				CACHE_MK_BADDR(cp, repl->tag, set),
				cp->bsize, repl, now+lat);
		  break;
		  case dtlb_access_fn_i:
		  lat += Sim->dtlb_access_fn(Write,
				CACHE_MK_BADDR(cp, repl->tag, set),
				cp->bsize, repl, now+lat);
		  break;
	  }
 ...
 */
  ...
  /* read data block */
  	switch (cp->pfs){
		  case dl1_access_fn_i:
		  lat += Sim->dl1_access_fn(Read, CACHE_BADDR(cp, addr), 
		  			cp->bsize, repl, now+lat);
		  break;
		  case dl2_access_fn_i:
		  lat += Sim->dl2_access_fn(Read, CACHE_BADDR(cp, addr), 
		  			cp->bsize, repl, now+lat);
		  break;
		  case il1_access_fn_i:
		  lat += Sim->il1_access_fn(Read, CACHE_BADDR(cp, addr), 
		  			cp->bsize, repl, now+lat);
		  break;
		  case il2_access_fn_i:
		  lat += Sim->il2_access_fn(Read, CACHE_BADDR(cp, addr), 
		  			cp->bsize, repl, now+lat);
		  break;
		  case itlb_access_fn_i:
		  lat += Sim->itlb_access_fn(Read, CACHE_BADDR(cp, addr), 
		  			cp->bsize, repl, now+lat);
		  break;
		  case dtlb_access_fn_i:
		  lat += Sim->dtlb_access_fn(Read, CACHE_BADDR(cp, addr), 
		  			cp->bsize, repl, now+lat);
		  break;
	  }
/* 
...
cache::cache_probe(struct cache_t *cp,		/* cache instance to probe */
...
cache::cache_flush(struct cache_t *cp,		/* cache instance to flush */
...
cache::cache_flush_addr(struct cache_t *cp,	/* cache instance to flush */
