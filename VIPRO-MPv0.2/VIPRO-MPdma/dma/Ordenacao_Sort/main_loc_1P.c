...
int main(){
	...
	for(i = 1; i < x; i++){
		vet_aux = 0x90000000;
		vet_aux[0] = 0x80000000+(i*parts*4);
		vet_aux[1] = sizeof(int);
		vet_aux[2] = parts;
		vet_aux[3] = 0; 		// 3ª position of the channel: 0 -> read in addr/ 1 -> write in addr
		vet_aux[4] = ID;
		...
		if(i == 2)
			mergesort(0,((i*parts)-1),vet);
// 		
	}

	while(int_cont < x);
	quicksort(vet, (x-1)*parts, tamanho-1); //first vector in the memory	
	mergesort((2*parts),(tamanho-1),vet);
	mergesort(0,(tamanho-1),vet);
// 	for(x = 0; x < tamanho; x++){
// 		printf(" > %d",vet[x]);
// 	}
	vet_aux = 0x90000000;
	vet_aux[0] = &vet[0];//0x80000000;
	vet_aux[1] = sizeof(int);
	vet_aux[2] = tamanho;
	vet_aux[3] = 0; 		// 3ª position of the channel: 0 -> read in addr/ 1 -> write in addr
	vet_aux[4] = ID;
	vet_aux[5] = 0x80000000;//(&vet[0])+parts*i;
	vet_aux[6] = 1; 	// 1 indica que o canal foi setado 2 indica q o dma o esta usando 0 indica não uso
	vet_aux = 0x90001000;	// aciona o dma_access
	vet_aux[0] = ID;		//indica qual é o channel solicitante
	while(int_cont == x);
