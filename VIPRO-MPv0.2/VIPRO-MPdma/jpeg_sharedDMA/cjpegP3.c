...
*/
  ...
  printf("P2: wainting for load image in shared memory\n");
  fflush(stdout);
...
//	  printf("input_done P2: %d", *input_done);
  ...
  {
  case 4:
	  /*cinfo.next_scanline = cinfo.image_height/2;
	  image_pos = cinfo.image_width* cinfo.num_components*cinfo.next_scanline;
	  while (cinfo.next_scanline < (cinfo.image_height*3)/4) {
	  ...
	  break;
  case 8:
	  /*cinfo.next_scanline = cinfo.image_height/4;
	  image_pos = cinfo.image_width* cinfo.num_components*cinfo.next_scanline;
	  while (cinfo.next_scanline < (cinfo.image_height*3)/8) {
		  num_scanlines = (*src_mgr->get_pixel_rows) (&cinfo, src_mgr);
		  (void) jpeg_write_scanlines(&cinfo, src_mgr->buffer, num_scanlines);
	  }*/
	  cinfo.next_scanline = cinfo.image_height/4;
