...
extern void compress(j_compress_ptr cinfo, int n_processors);
...
int buffer_size = 16;//from 1 to 128 2^n 
...
main (int argc, char **argv)
{
 ...
 /*
  ...
  char *arg3= "-opt";
  char *arg4= "-outfile";
  char *arg5= "output_small_encode.jpeg" ;
  char *arg6= "input_small.ppm";
*/
...
//  *num_processors = 2;
  ...
  /* MSG */
  *num_processors = atoi(argv[--argc]);
  argv[argc][0] = '\0';

//  printf("argc: %d, argv[argc]: %s, num_processors: %d\n", argc, argv[argc], *num_processors);
  ...
  flags[0] = 1;
  printf("P0: started encode\n");
  fflush(stdout);

  /* Process data */
  /* MSO: Process just a half image.
  ...
  image = (U_CHAR *)malloc(sizeof(U_CHAR*)*image_size);
	 ...
	 /*while (cinfo.next_scanline < cinfo.image_height) {
	  ...
	  while (cinfo.next_scanline < cinfo.image_height) {
		...
		if(int_cont == 0){
			...
			printf("P1 cont_interrupt %d\n", cont_interrupt);
		}
	  }
	  break;
  case 2:
	  /*while (cinfo.next_scanline < cinfo.image_height/2) {
		  num_scanlines = (*src_mgr->get_pixel_rows) (&cinfo, src_mgr);
		  (void) jpeg_write_scanlines(&cinfo, src_mgr->buffer, num_scanlines);
	  }*/
	  ping_pong = 0;
	  ...
	  // wait for the P1 finish, ok, ok, I know we need an interrupt here
	  printf("\nP0: waiting for slave processor\n");
	  while (flags[1] == 0){
		  // printf("flag_done P1: %d \n", *flag_done);
		  fflush(stdout);
	  }
	  break;
  case 4:
	  /*while (cinfo.next_scanline < cinfo.image_height/4) {
		num_scanlines = (*src_mgr->get_pixel_rows) (&cinfo, src_mgr);
		(void) jpeg_write_scanlines(&cinfo, src_mgr->buffer, num_scanlines);
	  }*/
	  ping_pong = 0;
	  ...
	  printf("\nP0: waiting for slaves processors\n");
	  while (((flags[1] == 0) ||
		  (flags[2] == 0))||
		  (flags[3] == 0)){
		  //printf("flag_done");
	  ...
	  break;
  case 8:
	  /*while (cinfo.next_scanline < cinfo.image_height/8) {
	  	num_scanlines = (*src_mgr->get_pixel_rows) (&cinfo, src_mgr);
	  	(void) jpeg_write_scanlines(&cinfo, src_mgr->buffer, num_scanlines);
	  }*/
	  ping_pong = 0;
