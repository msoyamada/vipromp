...
symbol::symbol(sim *S)
{
		...
		sym_datasyms_by_name = NULL;

		/* symbols loaded? */
		/*static*/syms_loaded = FALSE;	
}
...
symbol::flags2str(unsigned int flags)	/* bfd symbol flags */
{
  static char buf[256];
  char *p;
  ...
  *p = '\0';
  ...
  if (p == buf)
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "no flags detected");
...
static int
acmp(const void *sym1, const void *sym2)
{
  return (int)((*(struct sym_sym_t **)sym1)->addr - (*(struct sym_sym_t **)sym2)->addr);
}
...
/* qsort helper function */
static int
ncmp(const void *sym1, const void *sym2)
{
  return strcmp((*(struct sym_sym_t **)sym1)->name, (*(struct sym_sym_t **)sym2)->name);
}   

/* load symbols out of FNAME */
void
symbol::sym_loadsyms(char *fname,	/* file name containing symbols */
	     int load_locals)	/* load local symbols */
{
  ...
  if (!(abfd = bfd_openr(fname, "ss-coff-big")))
    if (!(abfd = bfd_openr(fname, "ss-coff-little")))
      Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "cannot open executable `%s'", fname);
  ...
  if (abfd->xvec->byteorder_big_p != (unsigned)ld_target_big_endian)
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "binary endian changed");
  ...
  if ((bfd_get_file_flags(abfd) & (HAS_SYMS|HAS_LOCALS)))
    {
      ...
      if (storage <= 0)
	Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "HAS_SYMS is set, but `%s' still lacks symbols", fname);
      ...
      for (i=0; i < nsyms; i++)
	{
      ...
      if (sym_nsyms <= 0)
	Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "`%s' has no text or data symbols", fname);
	...
	Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "could not locate all counted symbols");
  ...
  if (!bfd_close(abfd))
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "could not close executable `%s'", fname);
  ...
  if (fread(&fhdr, sizeof(struct ecoff_filehdr), 1, fobj) < 1)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "cannot read header from executable `%s'", fname);

  /* record endian of target */
  if (fhdr.f_magic != ECOFF_EB_MAGIC && fhdr.f_magic != ECOFF_EL_MAGIC)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "bad magic number in executable `%s'", fname);

  if (fread(&ahdr, sizeof(struct ecoff_aouthdr), 1, fobj) < 1)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "cannot read AOUT header from executable `%s'", fname);
  ...
  if (fread(&symhdr, sizeof(struct ecoff_symhdr_t), 1, fobj) < 1)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "could not read symbolic header from executable `%s'", fname);

  if (symhdr.magic != ECOFF_magicSym)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "bad magic number (0x%x) in symbolic header", symhdr.magic);
  ...
  if (fread(strtab, len, 1, fobj) < 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "error while reading symbol table names");
  ...
  if (fread(extr, sizeof(struct ecoff_EXTR), symhdr.iextMax, fobj) < 0)
    Sim->MISC->_fatal(__FILE__, __FUNCTION__, __LINE__, "error reading external symbol entries");
  ...
  for (i=0; i < symhdr.iextMax; i++)
	...
	case ECOFF_stStatic:
	  /* from data segment */
	  sym_db[sym_nsyms].name = Sim->MISC->mystrdup(&strtab[str_offset]);
  ...
  /* sanity check */
  if (debug_cnt != sym_nsyms)
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "could not locate all symbols");

  /* sort by address */
  qsort(sym_syms, sym_nsyms, sizeof(struct sym_sym_t *), acmp);

  /* sort by name */
  qsort(sym_syms_by_name, sym_nsyms, sizeof(struct sym_sym_t *), ncmp);
  ...
  if (debug_cnt != sym_ntextsyms)
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "could not locate all text symbols");

  /* sort by address */
  qsort(sym_textsyms, sym_ntextsyms, sizeof(struct sym_sym_t *), acmp);

  /* sort by name */
  qsort(sym_textsyms_by_name, sym_ntextsyms,
	sizeof(struct sym_sym_t *), ncmp);
  ...
  if (debug_cnt != sym_ndatasyms)
    Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "could not locate all data symbols");
      
  /* sort by address */
  qsort(sym_datasyms, sym_ndatasyms, sizeof(struct sym_sym_t *),acmp);
  ...
  for (i=0; i<sym_ntextsyms; i++)
	...
	(i != (sym_ntextsyms - 1)
	 ...
	 : ((Sim->LOADER->ld_text_base + Sim->LOADER->ld_text_size) - sym_textsyms[i]->addr));
    }
  for (i=0; i<sym_ndatasyms; i++)
	...
	(i != (sym_ndatasyms - 1)
	 ...
	 : ((Sim->LOADER->ld_data_base + Sim->LOADER->ld_data_size) - sym_datasyms[i]->addr));
    }
...
void
symbol::sym_dumpsym(struct sym_sym_t *sym,	/* symbol to display */
...
void
symbol::sym_dumpsyms(FILE *fd)			/* output stream */
{
  int i;
...
void
symbol::sym_dumpstate(FILE *fd)			/* output stream */
{
  int i;
...
symbol::sym_bind_addr(md_addr_t addr,		/* address of symbol to locate */
	      ...
	      enum sym_db_t db)		/* symbol database to search */
{
  ...
  switch (db)
    ...
    case sdb_data:
      ...
      break;
    default:
      Sim->MISC->_panic(__FILE__, __FUNCTION__, __LINE__, "bogus symbol database");
    }
...
/* bind name NAME to a symbol in symbol database DB, the index of the symbol
   ...
   non-NULL */
struct sym_sym_t *				/* symbol found, or NULL */
symbol::sym_bind_name(char *name,			/* symbol name to locate */
	      int *pindex,			/* ptr to index result var */
	      enum sym_db_t db)			/* symbol database to search */
{
